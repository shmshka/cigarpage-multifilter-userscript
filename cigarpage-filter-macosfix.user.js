// ==UserScript==
// @sandbox      raw
// @name         CigarPage Multi-Filter (macOS Fix)
// @namespace    cigarpage-filter
// @version      1.7.1-macosfix
...
(function () {
  ...
  // --- macOS Fix: Xray-safe page document/window (Tampermonkey Instant mode) ---
  // Tampermonkey on Firefox/macOS with `Instant` inject mode keeps the script
  // in the content-script sandbox behind Firefox's Xray wrapper. There, the
  // sandbox `document` can appear empty while the real page DOM lives in
  // `wrappedJSObject.document` / `unsafeWindow.document`. This alias unwraps
  // to the real page document when available, with safe fallbacks so
  // Chrome / Windows / Violentmonkey (where wrappedJSObject is undefined)
  // keep using the original `document`/`window` unchanged.
  const pageDoc = (() => {
    try {
      if (typeof wrappedJSObject !== 'undefined' && wrappedJSObject && wrappedJSObject.document) return wrappedJSObject.document;
    } catch (_) {}
    try {
      if (typeof unsafeWindow !== 'undefined' && unsafeWindow && unsafeWindow.document) return unsafeWindow.document;
    } catch (_) {}
    return document;
  })();
  const pageWin = (() => {
    try {
      if (typeof wrappedJSObject !== 'undefined' && wrappedJSObject) return wrappedJSObject;
    } catch (_) {}
    try {
  ...
  const pageLS = (() => {
    try { return pageWin.localStorage; } catch (_) { try { return window.localStorage; } catch (__) { return null; } }
  })();
  // One-time diagnostic — makes the next Mac report self-diagnosing without a second round-trip.
  try {
    const gmInfo = typeof GM_info !== 'undefined' ? (GM_info.scriptHandler + ' ' + (GM_info.version || '')) : 'n/a';
    console.log('[CigarPage Filter] init — xray=' + (pageDoc !== document) + ' href=' + pageWin.location.href + ' GM=' + gmInfo);
  } catch (_) {}
  ...
  function loadState() {
    try {
      const raw = pageLS ? pageLS.getItem(STORAGE_KEY) : null;
      if (raw) {
  ...
  function saveState() {
    try {
      if (pageLS) pageLS.setItem(STORAGE_KEY, JSON.stringify(state));
    } catch (_) {}
  }
  ...
  function getProductRowEntries() {
    const entries = [];
    for (const tr of pageDoc.querySelectorAll('table.cigar-grid tbody tr')) {
      entries.push({ el: tr, hideTarget: tr });
    }
    for (const item of pageDoc.querySelectorAll('#grouped-items-container .default-item')) {
  ...
  function injectStyles() {
    const style = pageDoc.createElement('style');
...
`;
    pageDoc.head.appendChild(style);
  }

  function buildFooterLinks() {
    const footer = pageDoc.createElement('div');
    footer.className = 'cp-footer';
    const share = pageDoc.createElement('a');
    ...
    const buy = pageDoc.createElement('a');
  ...
  function buildPanel() {
    const existing = pageDoc.getElementById('cp-filter-panel');
    if (existing) existing.remove();
    const strayBadge = pageDoc.getElementById('cp-filter-badge');
    if (strayBadge) strayBadge.remove();

    const container = pageDoc.createElement('div');
    container.id = 'cp-filter-panel';
    if (state.collapsed) container.classList.add('cp-collapsed');

    const dragHandle = pageDoc.createElement('div');
    ...
    const fontControls = pageDoc.createElement('span');
    fontControls.className = 'cp-font-controls';
    const increaseBtn = pageDoc.createElement('button');
    ...
    const decreaseBtn = pageDoc.createElement('button');
    ...
    const spacer = pageDoc.createElement('span');
    spacer.style.cssText = 'flex:1;';
    const enabledBtn = pageDoc.createElement('button');
    ...
    const collapseBtn = pageDoc.createElement('button');
    ...
    const copyBtn = pageDoc.createElement('button');
    ...
    const tabBar = pageDoc.createElement('div');
    tabBar.className = 'cp-tab-bar';
    for (const cfg of TAB_CONFIG) {
      const tab = pageDoc.createElement('div');
    ...
    const overallCount = pageDoc.createElement('span');
    overallCount.className = 'cp-tab-count';
    tabBar.appendChild(overallCount);

    const actions = pageDoc.createElement('div');
    actions.className = 'cp-actions';
    const expandBtn = pageDoc.createElement('button');
    ...
    const soldOutBtn = pageDoc.createElement('button');
    ...
    const alphaBtn = pageDoc.createElement('button');
    ...
    pageDoc.body.appendChild(container);
    panel = container;
  ...
  function ensureContextMenu() {
    ...
    contextMenu = pageDoc.createElement('div');
    contextMenu.id = 'cp-context-menu';

    const previewItem = pageDoc.createElement('div');
    ...
    const purgeItem = pageDoc.createElement('div');
    ...
    const ignoreItem = pageDoc.createElement('div');
    ...
    pageDoc.body.appendChild(contextMenu);
    return contextMenu;
  }
  ...
  function openBrandContextMenu(brand, x, y) {
    ...
    const left = Math.min(x, pageWin.innerWidth - rect.width - 4);
    const top = Math.min(y, pageWin.innerHeight - rect.height - 4);
  ...
  pageDoc.addEventListener('click', e => {
    if (contextMenu && !contextMenu.contains(e.target)) closeBrandContextMenu();
  });
  pageDoc.addEventListener('keydown', e => {
  ...
  function setupDrag(handle, container) {
    ...
    function getZoom(el) {
      const z = parseFloat(pageWin.getComputedStyle(el).zoom);
  ...
  function renderTab(tabId) {
    ...
    if (values.length === 0 && purged.length === 0 && ignored.length === 0) {
      const msg = pageDoc.createElement('div');
    ...
    if (ignored.length > 0) {
      const divider = pageDoc.createElement('div');
  ...
  function buildManagedBrandRow(brand, tooltip) {
    const row = pageDoc.createElement('div');
    ...
    label.title = tooltip;
    const restoreBtn = pageDoc.createElement('button');
  ...
  function buildCheckboxItem(tabId, raw, unchecked) {
    const item = pageDoc.createElement('label');
    item.className = 'cp-checkbox-item';

    const cb = pageDoc.createElement('input');
    cb.type = 'checkbox';
    ...
    const count = pageDoc.createElement('span');
  ...
  function renderRangeControl(config) {
    ...
    if (minBound === maxBound) {
      const onlyValue = pageDoc.createElement('div');
    ...
    const rangeMax = pageDoc.createElement('input');
    rangeMax.type = 'range';
    ...
    const clearBtn = pageDoc.createElement('button');
  ...
  function updateRangeValues(config, valuesElement) {
    ...
    const minLabel = pageDoc.createElement('span');
    minLabel.textContent = 'Min: ' + config.format(minValue);
    const maxLabel = pageDoc.createElement('span');
  ...
  function removeAllDetails() {
    for (const details of pageDoc.querySelectorAll('.injected_by_userscript')) {
      const spacer = details.previousElementSibling;
      if (spacer && spacer.classList.contains('cigar-details-spacer')) spacer.remove();
      details.remove();
    }
    for (const btn of pageDoc.querySelectorAll('table.cigar-grid .popoverBtn')) btn.style.display = '';
    for (const btn of pageDoc.querySelectorAll('table.cigar-grid .prodDetailsBtn')) btn.style.display = '';
    for (const toggle of pageDoc.querySelectorAll('.cigar-attr-toggle')) toggle.style.display = 'none';
    for (const tr of pageDoc.querySelectorAll('table.cigar-grid tbody tr')) tr.style.position = '';
  }

  function expandAllBlendInfo() {
    const toggles = pageDoc.querySelectorAll('.cigar-attr-toggle');
    let clicked = 0;
    for (const toggle of toggles) {
      if (pageWin.getComputedStyle(toggle).display !== 'none') continue;
      const productId = toggle.id.replace('cigarAttrToggler_', '');
      const btn = pageDoc.querySelector('.prodDetailsBtn[data-productid="' + productId + '"]');
      if (btn) {
        btn.click();
        clicked++;
      }
  ...
  function injectAllDetails() {
    const tables = pageDoc.querySelectorAll('table.cigar-grid');
    let injected = 0;
    for (const table of tables) {
      ...
      for (const tr of rows) {
        ...
        const details = pageDoc.createElement('div');
  ...
  function positionInjectedDetails(details) {
    ...
    if (!spacer || !spacer.classList.contains('cigar-details-spacer')) {
      spacer = pageDoc.createElement('div');
  ...
  function waitForTables(maxAttempts, interval, callback) {
    ...
    function check() {
      const tables = pageDoc.querySelectorAll('table.cigar-grid');
      const items = pageDoc.querySelectorAll('#grouped-items-container .default-item');
      const hasData = tables.length > 0 || items.length > 0;
      if (hasData) {
        callback(true);
  ...
  function init() {
    ...
    const isGroupedPage = pageDoc.querySelector('#grouped-items-container') !== null;
    waitForTables(isGroupedPage ? 120 : 40, 250, function (found) {
      if (!found) {
        console.log('[CigarPage Filter] No product tables found on this page. — tables=' + pageDoc.querySelectorAll('table.cigar-grid').length + ' labels=' + pageDoc.querySelectorAll('.cigar-attr-label').length + ' items=' + pageDoc.querySelectorAll('#grouped-items-container .default-item').length + ' href=' + pageWin.location.href + ' xray=' + (pageDoc !== document) + ' GM=' + (typeof GM_info !== 'undefined' ? (GM_info.scriptHandler + ' ' + (GM_info.version || '')) : 'n/a'));
        return;
      }
      ...
      updateBadgeText();
      setupObserver();
      pageWin.addEventListener('resize', () => relayoutInjectedDetails());
    });
  }

  if (pageDoc.readyState === 'loading') {
    pageDoc.addEventListener('DOMContentLoaded', init);
  } else {
    init();
  }
})();
